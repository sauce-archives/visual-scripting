...
import org.testobject.kernel.imgproc.util.GaussianFilter;

/**
 * Modified version based on following publications:
 *
 * http://www.cs.brown.edu/~pff/papers/seg-ijcv.pdf
 * http://www.cs.brown.edu/~pff/segment/
 *
 * Added hierarchical extraction of segments.
 *
 * @author enijkamp 
 */
public class GraphBlobBuilder {

	private static final int ROOT = 0, UNDEFINED = -1;
	private static final float SIGMA = 0.5f;
	private static final double THRESHOLD = 4500;
	...
	public GraphBlobBuilder(int width, int height) {
		this(width, height, SIGMA, THRESHOLD);
	}
	...
	private Blob[] createHierarchy(int[] pixel, int numberOfSegments) {
		...
		blobs[0] = new Blob(ROOT, new Rectangle(0, 0, width, height), 0, new LinkedList<Blob>(), ids);
		...
		for (int i = 1; i < blobs.length; i++) {
			...
			final Rectangle bbox = new Rectangle(segment_left[i], segment_top[i], width, height);
	...
	private void setParent(int child, int parent, int[] parents) {
		...
		// second parents 'tells' that he is childs father, thus look for convergence of parent paths
		for (int childParent = child; childParent != ROOT; childParent = parents[childParent]) {
			for (int parentParent = parent; parentParent != ROOT; parentParent = parents[parentParent]) {
