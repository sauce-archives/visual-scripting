 ...
 *
 */
public class GroupBuilder<T extends BoundingBox>
{
	...
	*/
	...
	public List<Group<T>> buildGroups(List<T> list, Insets insets)
	{
		...
		{
			//            System.out.println("(RE)ALLOCATING INTERNAL MEMORY");
		...
		for (T b : list)
		{
			...
			ops[idx * 2 + 1] = new Op(r.x + r.width - 1 + insets.right, idx + 1, false);
		...
		for (int i = 0; i < 2 * length; i++)
		{
			...
			int end = r.y + r.height - 1 + insets.bottom;
		...
		//        if(out.size() > 0) System.out.println("Detected + " + out.size() + " groups");
		return out;
	}
}
