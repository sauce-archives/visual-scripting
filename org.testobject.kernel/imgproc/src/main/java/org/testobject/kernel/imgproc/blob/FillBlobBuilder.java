...
public class FillBlobBuilder
{
    ...
    {
        this.raster = raster;
        this.ids = new int[raster.getSize().height][raster.getSize().width];
    }
    ...
    private List<Blob> process(int count, boolean color)
    {
        ...
        while (count > 0)
        {
            ...
            Rectangle minmax = new Rectangle(point.x, point.y, 1, 1);
    ...
    private int floodFill(int count, Rectangle limits, boolean color, int id)
    {
        ...
        while (count > 0)
        {
            ...
            for (Direction dir : directions)
            {
                ...
                if (probe.x >= 0 && probe.x < size.width && probe.y >= 0 && probe.y < size.height && ids[probe.y][probe.x] == 0)
                {
        ...
        limits.x = minx;
        limits.y = miny;
        limits.width = maxx - minx + 1;
        limits.height = maxy - miny + 1;
    ...
    {
        int totalArea = raster.getSize().width * raster.getSize().height;
        
        int count = bootstrap();

        int area = floodFill(count, new Rectangle(), false, blobIdSource++); // note blob id '1'
        ...
        return new Blob(1, new Rectangle(0, 0, size.width, size.height), area, blobs, ids);
    }
