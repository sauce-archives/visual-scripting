 ...
 *
 */
public class LinearColorBlobBuilder
{
	public static final Log log = LogFactory.getLog(LinearColorBlobBuilder.class);
	...
	//  if BG:
	//  possible merge between a and c (need to check only if b is FG (positive))
	public Blob build(Image.Int r)
	{
		int idSource = 1;
		int merges = 0;

		final int width = r.w;
		final int height = r.h;
		...
		for (int y = 0; y < height; y++)
		{
			...
			System.arraycopy(raster, y*width, row, 0, width);
			
			int[] previousRow = y > 0 ? new int[width] : null;
			if(y > 0) System.arraycopy(raster, (y-1)*width, previousRow, 0, width);
			...
			{
				c = previousIdRow[0];
				c_color = previousRow[0];
				if (width > 1) {
					d = previousIdRow[1];
					d_color = previousRow[1]; 
				} else {
					d = -1;
					d_color = -1;
				}
			...
			for (int x = 0; x < width; x++)
			{
				...
				if (a > 0 && dot_color == a_color)
				{
					newa = a;

					// check for blob merge
					if (d != b && d != c && d > 0 && a_color == d_color && a != d)
					{
						merge(newa, d);
						merges++;
					}
				}
				else if (b > 0 && dot_color == b_color)
				{
					newa = b;
				}
				else if (c > 0 && dot_color == c_color)
				{
					newa = c;
				}
				else if (d > 0 && dot_color == d_color)
				{
					newa = d;
				}
				else
		...
		// beyond last scanline - possibly merge blobs
		for (int x = 0; x < width; x++)
		{
	...
	{
		LinkedList<Blob> ch = new LinkedList<>();
