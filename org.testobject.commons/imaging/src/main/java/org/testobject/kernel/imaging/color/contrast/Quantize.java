...
public class Quantize {
	...
	public static Image.Int quantize(Image.Int source, int[] colors) {
		
		int[][] palette = toPalette(colors);
		Image.Int target = org.testobject.commons.util.image.ImageUtil.clone(source);
		for(int i = 0; i < source.pixels.length; i++) {
			target.pixels[i] = toNearestColor(source.pixels[i], palette);
		}
	...
	private static int toNearestColor(int color, int[][] palette) {
		
		final int[] colorRgb = ImageUtil.toRGB(color);
		
		int[] bestRgb = {};
		double bestDistance = Double.MAX_VALUE;
		
		for(int[] paletteRgb : palette) {
			double distance = squaredEuclidean(colorRgb, paletteRgb);
		...
		return ImageUtil.toInt(bestRgb);
	}

	private static double squaredEuclidean(int[] a, int[] b) {
		double sum = 0;
	...
	public static int[][] toPalette(int[] colors) {
		int[][] rgbs = new int[colors.length][3];
		for(int i = 0; i < colors.length; i++) {
			rgbs[i] = new int[] { (colors[i] >> 16) & 0xff, (colors[i] >> 8) & 0xff, (colors[i] >> 0) & 0xff };
		}
		return rgbs;
	}
	...
	public static Image.Int quantize(Image.Int source, int colors) {
		...
		final int[] palette = Quantize.quantizeImage(pixels, colors);
		
		for(int i = 0; i < target.pixels.length; i++) {
			final int argb = target.pixels[i];
			final int r = (argb >> 16) & 0xff;
			final int g = (argb >> 8) & 0xff;
			final int b = (argb >> 0) & 0xff;
			final int rgb = (r << 16) | (g << 8) | (b << 0);
			target.pixels[i] = palette[rgb];
		}

		return target;
	}

	public static BufferedImage to256Colors(BufferedImage image)
			throws IOException {
		...
		for (int x = 0; x < image.getWidth(); x++) {
			for (int y = 0; y < image.getHeight(); y++) {
				image.setRGB(x, y, palette[pixels[x][y]]);
			}
		}
	...
	public static BufferedImage to8Colors(BufferedImage image)
			throws IOException {
		final int[][] pixels = ImageUtil.getPixels(image);
		final int[] palette = Quantize.quantizeImage(pixels, 16);
